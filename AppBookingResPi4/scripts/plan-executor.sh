...
#   ./scripts/plan-executor.sh test                  run build+test commands from config
...
usage() {
    ...
    echo "  $0 test                  Run build+test commands from config"
    exit 1
}
...
Example: "src/api.c POST routing is currently stubbed at line ~60."]
...
- [ ] **1.1** In `src/api.c`, locate `[symbol]` at line ~N.
...
**Pass:** `[native] Build successful` in output, exit code 0.
**Fail:** Copy the full compiler error and report it. Do not attempt to fix.

---

## T2 — [What is being verified]
...
cmd_list() {
    ...
    for folder in "$PLANS_DIR"/*/; do
        ...
        if [[ -f "$tasks_file" ]]; then
            ...
            printf "  %-50s  %s/%s tasks done\n" "$folder" "$done" "$total"
        else
            printf "  %-50s  (no tasks.md)\n" "$folder"
        fi
    done
